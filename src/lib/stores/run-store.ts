...
class RunStore {
  ...
  async createTestRun(
    ...
    try {
      ...
      const insertData = {
        ...
        browserbase_session_id: browserbase_session_id || null,
      };
  ...
  // Get Browserbase session ID for a test run
  async getBrowserbaseSessionId(testRunId: string): Promise<string | null> {
    try {
      ...
      if (error) {
        console.error("Error fetching browserbase_session_id:", error);
        return null;
      }

      return data?.browserbase_session_id || null;
    } catch (error) {
      console.error("Error in getBrowserbaseSessionId:", error);
      return null;
    }
  }
