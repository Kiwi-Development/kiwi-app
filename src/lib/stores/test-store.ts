...
class TestStore {
  ...
  async saveTest(test: Test): Promise<Test | null> {
    try {
      ...
      if (test.id && isUuid) {
        ...
        if (!data || !data.id) {
          console.error("Test created but no ID returned:", data);
          return null;
        }
        result = data;
      }

      const convertedTest = await this.dbToAppTest(result as DatabaseTest);
      if (!convertedTest || !convertedTest.id) {
        console.error("Test conversion failed - missing ID:", convertedTest);
        return null;
      }
  ...
  /**
   * Get test run by ID from API
   */
  async getTestRun(testRunId: string): Promise<any | null> {
    try {
  ...
  /**
   * Get report/findings for a test run
   */
  async getReport(testRunId: string): Promise<any | null> {
    try {
      const response = await fetch(`/api/reports/${testRunId}`);
      if (!response.ok) {
        if (response.status === 404) {
          return null;
        }
