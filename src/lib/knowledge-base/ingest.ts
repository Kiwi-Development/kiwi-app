...
function getSupabaseClient() {
  ...
  if (!supabaseUrl || !supabaseSecretKey) {
    throw new Error(
      "Missing required environment variables: NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SECRET_KEY/SUPABASE_SERVICE_ROLE_KEY"
    );
  }
 ...
 * Returns chunks without id (id is generated by database)
 */
function parseKnowledgeFile(
  filePath: string,
  category: KnowledgeChunk["category"]
): Omit<KnowledgeChunk, "id" | "similarity">[] {
  const content = fs.readFileSync(filePath, "utf-8");
  const chunks: Omit<KnowledgeChunk, "id" | "similarity">[] = [];
